//============================================================================
// Name        : hasar-test-c++.cpp
// Author      : Marcelo Carabajal
// Version     :
// Copyright   : Your copyright notice
// Description : HASAR - TEST C++
//============================================================================
...
int* climbingLeaderboard(int scores_count, int* scores, int alice_count, int* alice, int* result_count) {
    // eliminar puntajes duplicados
    int cont_puntajes_unicos = 1;
    for (int i = 1; i < scores_count; i++) {
        if (scores[i] != scores[i-1]) {
        	cont_puntajes_unicos++;
        }
    }
    int* puntajes_unicos = malloc(cont_puntajes_unicos * sizeof(int));
    puntajes_unicos[0] = scores[0];
    int j = 1;
    for (int i = 1; i < scores_count; i++) {
        if (scores[i] != scores[i-1]) {
        	puntajes_unicos[j] = scores[i];
            j++;
        }
    }

    // calcular rankings de Alice
    int* result = malloc(alice_count * sizeof(int));
    int i = cont_puntajes_unicos - 1;
    for (int j = 0; j < alice_count; j++) {
        // busco la posicion de Alice en la tabla de clasificacion
        while (i >= 0 && puntajes_unicos[i] < alice[j]) {
            i--;
        }
        if (i < 0) {
            result[j] = 1;
        } else if (puntajes_unicos[i] == alice[j]) {
            result[j] = i + 1;
        } else {
            result[j] = i + 2;
        }
